...
class FrameToPointOfDeploy():
    def __init__(self, cali_data_path, model_path):
        #首先加载onnx模型
        self.net1 = onnxruntime.InferenceSession(r"c:/Users/jchao/Desktop/net1.onnx")
        self.net2 = onnxruntime.InferenceSession(r"c:/Users/jchao/Desktop/net2.onnx")
        print("onnx模型加载完成")
        ...
        for i in range(n):
            ...
            #输入网络处理并将结果保存
            in_dict = {"leftEye_img": leftEye_img.astype(np.float32),
                    "rightEye_img": rightEye_img.astype(np.float32),
                    "face_img": face_img.astype(np.float32),
                    "inputRects": inputRects.astype(np.float32)}
            self.prepossed_imgs.append(self.net1.run(['ret'], in_dict)[0])
        print("微调图片处理完毕")
        ...
        for i in range(n):
            for j in range(i + 1, n):
                diff = self.net2.run(['ret'], {'x1': self.prepossed_imgs[i], 'x2': self.prepossed_imgs[j]})[0].squeeze()
                # if abs(diff[0]) + abs(diff[1]) < 18.5:
                self.kx += (self.labels[i][0] - self.labels[j][0]) / diff[0]
                self.ky += (self.labels[i][1] - self.labels[j][1]) / diff[1]
                divided += 1
    ...
    def frameToPoint(self, frame, rects):
        ...
        in_dict = {"leftEye_img": leftEye_img.astype(np.float32),
                   ...
                   "face_img": face_img.astype(np.float32),
                   "inputRects": inputRects.astype(np.float32)}
        x1 = self.net1.run(['ret'], in_dict)[0]

        gaze1_x, gaze1_y, w_sum = 0, 0, 0
        #两两之间求一次差值
        for i, x2 in enumerate(self.prepossed_imgs):
            gaze2_x, gaze2_y = self.labels[i][0], self.labels[i][1]
            diff = self.net2.run(['ret'], {'x1': x1, 'x2': x2})[0].squeeze()
            # print(diff)
...
#     util1 = FrameToPointOfDeploy(r"C:\Users\jchao\Desktop\calibrationDataset\gagajie2", r"./Iter_10_jie2.pt")
#     util2 = FrameToPoint(r"C:\Users\jchao\Desktop\calibrationDataset\gagajie2", r"./Iter_10_jie2.pt")
#
#     img = cv2.imread(r"C:\Users\jchao\Desktop\calibrationDataset\jie\1.png")
#     parameter, ear = getRectOfDlib(img)
#
#     t1 = timeit.timeit(lambda : util1.frameToPoint(img, parameter), number=100)
#     t2 = timeit.timeit(lambda : util2.frameToPoint(img, parameter), number=100)
#     print("部署：", t1)
#     print("原生：", t2)
...
if __name__ == "__main__":
    util = FrameToPointOfDeploy(r"C:\Users\jchao\Desktop\calibrationDataset\gagajie2", r"./Iter_10_jie2.pt")
