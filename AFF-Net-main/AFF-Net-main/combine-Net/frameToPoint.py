...
def getPar(img, choice):
    ...
    # rects = getRectOfRec(img)
    ret = getRectOfDlib(img)   #在速度和准确度之间取一个trade-off
    ...
    return leftEye_img, rightEye_img, face_img, rects, ear
...
class FrameToPoint():
    def __init__(self, cali_data_path, model_path):
        ...
        for i in range(n):
            ...
            #拿到组件，输入网络处理
            leftEye_img, rightEye_img, face_img, inputRects, _ = getPar(img, "rec")
    ...
    def frameToPoint(self, frame):
        ...
        if isinstance(ret, type(None)):
            return None
        leftEye_img, rightEye_img, face_img, inputRects, ear = ret
        # 处理成可以直接输入网络的形式
        face_img = torch.from_numpy(face_img).type(torch.FloatTensor).to(self.device)
        leftEye_img = torch.from_numpy(leftEye_img).type(torch.FloatTensor).to(self.device)
        rightEye_img = torch.from_numpy(rightEye_img).type(torch.FloatTensor).to(self.device)
    ...
    #         diff = self.net2(x1, x2).cpu().detach().numpy().squeeze()
    #
    #         if abs(diff[0]) + abs(diff[1]) < minDiff:
    #             minDiff = abs(diff[0]) + abs(diff[1])
    #             gaze1_x = gaze2_x + self.kx * diff[0]
    #             gaze1_y = gaze2_y + self.ky * diff[1]
    #
    #     print(minDiff)
    #     return gaze1_x, gaze1_y

    # # 找最近n个做平均
    # def frameToPoint(self, frame, n=4):
    #     # 首先对frame做处理
    #     # 拿到组件，输入网络处理
    #     # face_img, leftEye_img, rightEye_img, inputRects, _ = getInput(frame, None)
    #     ret = getPar(frame, "cv")
    ...
    #     x1 = self.net1(leftEye_img, rightEye_img, face_img, inputRects)
    #
    #     save_list = []  #这个列表用来存放全部的差值和对应的gaze
    #     # 两两之间求一次差值
    #     for i, x2 in enumerate(self.prepossed_imgs):
    #         gaze2_x, gaze2_y = self.labels[i][0], self.labels[i][1]
    #         diff = self.net2(x1, x2).cpu().detach().numpy().squeeze()
    #         save_list.append((diff, gaze2_x, gaze2_y))
    #
    #     #对save_list中的内容按照diff绝对值之和从小到大排序
    #     save_list.sort(key=lambda x: abs(x[0][0]) + abs(x[0][1]))
    #
    #     #取前n个最小的求和
    #     gaze1_x, gaze1_y, w_sum = 0, 0, 0
    #     #两两之间求一次差值
    #     for i in range(n):
    #         diff, gaze2_x, gaze2_y = save_list[i]
    #         tmp_x = gaze2_x + self.kx * diff[0]
    #         tmp_y = gaze2_y + self.ky * diff[1]
    #         w = 1 / (abs(diff[0]) + abs(diff[1]))
    #
...
if __name__ == "__main__":
    util = FrameToPoint(r"C:\Users\jchao\Desktop\calibrationDataset\gagajie2", r"./checkpoint/Iter_10_jie2.pt")
    img = cv2.imread(r"C:\Users\jchao\Desktop\calibrationDataset\jie\1.png")
    timem = timeit.timeit(lambda : util.frameToPoint(img), number=100)
    t = timeit.timeit(lambda : getRectOfDlib(img), number=100)
    print(timem)
    print(t)
...
#     # util = FrameToPoint(r"C:\Users\jchao\Desktop\calibrationDataset\gagajie2", r"./checkpoint/Iter_20_zuhefeiwuceshi-Net.pt")
#     util = FrameToPoint(r"C:\Users\jchao\Desktop\calibrationDataset\gagajie2", r"./checkpoint/Iter_10_jie2.pt")
